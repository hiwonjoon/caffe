...
class SuperCategoryFCLayerTest: public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;
 protected:
  SuperCategoryFCLayerTest()
  ...
  virtual ~SuperCategoryFCLayerTest() { 
...
TYPED_TEST_CASE(SuperCategoryFCLayerTest, TestDtypesAndDevices);

TYPED_TEST(SuperCategoryFCLayerTest, TestSetUp) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  shared_ptr<SuperCategoryFCLayer<Dtype> > layer(
      new SuperCategoryFCLayer<Dtype>(layer_param));
...
TYPED_TEST(SuperCategoryFCLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
...
TYPED_TEST(SuperCategoryFCLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  bool IS_VALID_CUDA = false;
#ifndef CPU_ONLY
  IS_VALID_CUDA = CAFFE_TEST_CUDA_PROP.major >= 2;
  ...
  if (Caffe::mode() == Caffe::CPU ||
	...
	SuperCategoryFCLayer<Dtype> * layer = new SuperCategoryFCLayer<Dtype>(layer_param);
