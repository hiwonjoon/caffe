...
class AccuracyLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  int ignore_label_;
  /// Wheter print out top-k result. Used for making test result.
  bool print_;
};

/**
 * @brief Computes the classification accuracy and confusion matrix for a one-of-many
 *        classification task.
 */
template <typename Dtype>
class AccuracyWithConfusionLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides AccuracyParameter accuracy_param,
   *     with AccuracyLayer options:
   ...
   *     correct if the correct label is among the top 5 predicted labels.
   */
  explicit AccuracyWithConfusionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "AccuracyWithConfusion"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
 ...
 protected:
   ...
   *         \end{array} \right.
   *      @f$
   *   -# @f$ ({N} \times {N} \times 1 \times 1) @f$
   *      the computed confusion matrix: @f$
   *      @f$
   */
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
...
template <typename Dtype>
class LabelAccuracyLayer : public Layer<Dtype> {
 public:
  explicit LabelAccuracyLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "LabelAccuracy"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
 ...
 protected:
  ...
  /// @brief Not implemented -- TopLabelLayer cannot be used as a loss.
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
    for (int i = 0; i < propagate_down.size(); ++i) {
      if (propagate_down[i]) { NOT_IMPLEMENTED; }
...
template <typename Dtype>
class LabelAccuracyWithConfusionLayer : public Layer<Dtype> {
 public:
  explicit LabelAccuracyWithConfusionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "LabelAccuracyWithConfusion"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return 2; }
...
/**
 * @brief Computes the classification top-k result
 *        classification task.
 */
template <typename Dtype>
class TopLabelLayer : public Layer<Dtype> {
 public:
  explicit TopLabelLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "TopLabel"; }
