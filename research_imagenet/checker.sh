...
then
	...
	case ${answer:0:1} in
		y|Y )
			../../build/tools/caffe train -solver ./checker.prototxt -gpu $gpu_num -snapshot ./$list &> $filename.log &
		;;
		* )
			../../build/tools/caffe train -solver ./checker.prototxt -weights ../base.caffemodel -gpu $gpu_num &> $filename.log &
		;;
	esac
else
	../../build/tools/caffe train -solver ./checker.prototxt -weights ../base.caffemodel -gpu $gpu_num &> $filename.log &
